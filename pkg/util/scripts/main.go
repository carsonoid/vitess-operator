...
master_alias_json=$(/vt/bin/vtctlclient ${VTCTL_EXTRA_FLAGS[@]} -server $VTCTLD_SVC GetShard sharded_db/80-)
...
until [ $DONE_REPARENTING ]; do
  ...
  /vt/bin/vtctlclient ${VTCTL_EXTRA_FLAGS[@]} -server $VTCTLD_SVC PlannedReparentShard -keyspace_shard=sharded_db/80- -avoid_master=$current_alias
