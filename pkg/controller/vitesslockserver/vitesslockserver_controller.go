...
func (r *ReconcileVitessLockserver) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	err := r.client.Get(context.TODO(), request.NamespacedName, vl)
	if err != nil {
	...
	rr, err := ReconcileVitessLockserverObject(vl, reqLogger)
	// Update the vl in the cluster if needed
	return rr, err
}

// ReconcileVitessLockserverObject does all the actual reconcile work
func ReconcileVitessLockserverObject(vl *vitessv1alpha2.VitessLockserver, log logr.Logger) (reconcile.Result, error) {
	reqLogger := log.WithValues()
	reqLogger.Info("Reconciling VitessLockserver")
	// TODO actual reconcile
	vl.Status.State = "Ready"
	return reconcile.Result{}, nil
}
